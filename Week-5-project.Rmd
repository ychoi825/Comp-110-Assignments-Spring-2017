...
!!! line 75 won't run + keep on creating cache file
```{r}
plot_data <- ScorecardSmall %>%
  select(CCSIZSET, CONTROL, ADM_RATE, NPT41_PUB, NPT41_PRIV, NPT43_PRIV,NPT43_PUB, NPT45_PRIV, NPT45_PUB) %>%
  filter(CONTROL!=3) %>%
  gather(key = Quantile, value = Cost, NPT41_PUB, NPT41_PRIV, NPT43_PRIV,NPT43_PUB, NPT45_PRIV, NPT45_PUB) %>%
  mutate(Q1=NPT41_PUB, Q2=NPT42_PUB, Q3=NPT43_PUB, Q4=NPT44_PUB, Q5=NPT45_PUB, Q1=NPT41_PRIV, Q2=NPT42_PRIV, Q3=NPT43_PRIV, Q4=NPT44_PRIV, Q5=NPT45_PRIV)


ggplot(plot_data, aes(x = ADM_RATE, y = Cost)) + 
  geom_point(color = "blue", alpha = 0.05) + 
  facet_grid(CONTROL~Q) + 
  geom_smooth(color="black") + xlab("Admission Rate") + ylab("Cost")
```
